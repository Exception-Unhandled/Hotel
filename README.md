# Hotel
> #### This was my first project using Git in 2025.<br><br>
> At the time I was learning how to use Git and how to program in C#.
This was aswell my first "big" project on C# collaborating with a colleague.

<br>For the program itself, it features: 
- A reservation manager of a hotel; 
- A Importing reservations through a .txt file function;
- Inserting a new reservation;
- Searching for and showing existing reservations; 
- Editing, exporting and generating reservations;
- A list of available room slots;
- Checking room prices;
- Clearing a reservation

Everything on a Command Line Interface (CLI).<br>

Even though I didn't quite use the tools git offers properly, I did have the opportunity to understand the basis of it 
while having a taste of how to work on a project with others.<br><br>



This project uses .NET 8.0
